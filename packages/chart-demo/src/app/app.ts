...
import { TradeType } from '@pavel/trading'
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    try {
      ...
      const xMarkers: ExternalMarker[] = history.map(trade => ({
        ...
        x: trade.timestamp,
        color:
          trade.type === TradeType.BUY_USD
            ? 'hsl(161, 79%, 38%)'
            : 'hsl(42, 67%, 50%)',
        lineWidth:
          trade.type === TradeType.BUY_USD ? (2 as const) : (2 as const),
      }))
      ...
      //     const color = (() => {
      //       if (prevTrade.type === TradeType.BUY_BYN && trade.type === TradeType.BUY_USD) {
      ...
      //       if (prevTrade.type === TradeType.BUY_USD && trade.type === TradeType.BUY_BYN) {
